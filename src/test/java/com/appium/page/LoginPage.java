...
public class LoginPage extends BasePage {
    ...
    @iOSXCUITFindBy(iOSClassChain = "**/XCUIElementTypeButton[`name == '로그인'`]")
    private WebElement loginBtn;

    /** 로그인 실패 안내 텍스트 */
    @AndroidFindBy(uiAutomator = "new UiSelector().text(\"아이디 또는 패스워드가 일치하지 않습니다.\")")
    @iOSXCUITFindBy(iOSNsPredicate = "label == '아이디 또는 패스워드가 일치하지 않습니다.'")
    ...
    public void login(String id, String pw) {
        ...
        sendKeys(pwField, pw);
        click(loginBtn);
        // 로그인 성공 후 시스템 Alert 처리 (예: 아이콘 변경 알림)
